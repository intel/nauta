...
Submits training jobs. Use this command to submit single and multi-node training jobs (by passing –t parameter with a name of a multi-node pack), and multiple jobs at once (by passing –pr/-ps parameters).
...
### Options
 ...
 |`-e, --env TEXT` | No | Environment variable passed to training. A user can pass as many environmental variables, as desired. Each variable should be in passed as a separate `-e`  parameter.|
...
Should issues arise, a message (or messages) with a description of their cause (or causes) displays. Otherwise, the logs are filtered based on the command's parameters.
...
Launches a local browser with Jupyter* notebook. If a script's name is given as a parameter of a command, then this script
