...
The URL and authorization header for accessing TensorFlow Serving REST API will be shown after prediction instance is submitted, as in the following example (scroll right to see full contents).
...
Now you can access REST API on the following URL (scroll right to see full contents):
