...
## Experiments still in a RUNNING State (even if they finish with success
...
Till a resolution of the problem on library's side, monitor the statuses of experiments and check whether they _are not_ running too long relative to predicted duration. If there are such cases, cancel an experiment _without_ purging it. The results are still available on sharess. 
