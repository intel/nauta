...
def test_list_unitialized_experiments_in_cli_success(mocker, capsys):
    api_list_runs_mock = mocker.patch("commands.common.list_utils.Run.list")
    api_list_runs_mock.return_value = TEST_RUNS

    api_list_experiments_mock = mocker.patch("commands.common.list_utils.Experiment.list")
    api_list_experiments_mock.return_value = TEST_NONINITIALIZED_EXPERIMENTS

    get_namespace_mock = mocker.patch("commands.common.list_utils.get_kubectl_current_context_namespace")

    list_utils.list_unitialized_experiments_in_cli(verbosity_lvl=0, all_users=False, name="", listed_runs_kinds=[],
...
def test_list_experiments_success(mocker):
    api_list_runs_mock = mocker.patch("commands.common.list_utils.Run.list")
    api_list_runs_mock.return_value = TEST_RUNS
    mocker.patch("commands.common.list_utils.Experiment.get", return_value=TEST_EXPERIMENT)
    get_namespace_mock = mocker.patch("commands.common.list_utils.get_kubectl_current_context_namespace")

    list_utils.list_runs_in_cli(verbosity_lvl=0, all_users=False, name="", status=None, listed_runs_kinds=[],
...
def test_list_experiments_all_users_success(mocker):
    ...
    list_utils.list_runs_in_cli(verbosity_lvl=0, all_users=True, name="", status=None, listed_runs_kinds=[],
                                runs_list_headers=TEST_LIST_HEADERS, with_metrics=False, brief=False)
...
def test_list_experiments_failure(mocker):
    ...
    mocker.patch("commands.common.list_utils.Experiment.get", return_value=TEST_EXPERIMENT)
    get_namespace_mock = mocker.patch("commands.common.list_utils.get_kubectl_current_context_namespace")
    sys_exit_mock = mocker.patch("commands.common.list_utils.exit")

    list_utils.list_runs_in_cli(verbosity_lvl=0, all_users=False, name="", status=None,
                                listed_runs_kinds=[], runs_list_headers=TEST_LIST_HEADERS,
                                with_metrics=False, brief=False)
...
def test_create_fake_run():
    assert TEST_RUN == list_utils.create_fake_run(TEST_EXPERIMENT)


def test_replace_initalizing_runs_no_changes(mocker):
    mocker.patch("commands.common.list_utils.Experiment.get", return_value=TEST_EXPERIMENT)
    assert len(list_utils.replace_initializing_runs(TEST_RUNS)) == 2


def test_replace_initializing_runs_two_not_ready(mocker):
    get_experiment_mock = mocker.patch("commands.common.list_utils.Experiment.get", return_value=TEST_EXPERIMENT)
    assert len(list_utils.replace_initializing_runs(TEST_RUNS_CREATING)) == 5
