...
from commands.model import common
...
def test_show_logs_failure_proxy_problem(mocker, exception):
    ...
    assert list_operations_mock.call_count == 0, 'operation was retrieved'
    assert es_client_instance.get_argo_workflow_logs_generator.call_count == 0, 'operation logs retrieval was called'
    assert result.exit_code == 1
